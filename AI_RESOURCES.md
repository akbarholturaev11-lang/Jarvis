...
| `core/reminder_events.py` | Validates and atomically claims private scheduled-reminder events for Gemini Live speech. | HIGH | Spoken reminders duplicate, disappear, or fail to fall back safely. | Date parsing or OS scheduler registration is the only issue. |
...
| `actions/reminder.py` | Creates OS-native reminders, notifications, private speech events, and local speech fallback. | HIGH | Reminder scheduling or fallback speech fails on macOS/Windows/Linux. | Gemini event claiming is the issue; inspect `core/reminder_events.py` and `main.py` together. |
