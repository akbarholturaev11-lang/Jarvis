...
- Desktop settings window: a corner ⚙ gear in `ui.py` opens `SettingsOverlay` (remote on/off, Show QR/PIN, keep-awake, auto-start, clipboard actions, assistant identity, RU/EN language, revoke paired devices, connection status, macro builder) with an animated native `ToggleSwitch`. Wired to `main.py` via the single `on_settings_action(action, **kwargs)` callback. The desktop stays native PyQt6; Unlumen UI is animation reference only (web-only per `AI_RULES.md`).
- Auto-start on login (ported/adapted from upstream Mark-XLIX): cross-platform through `core/platform_adapters` (`autostart_status`/`set_autostart` contract — macOS LaunchAgent plist, Windows HKCU Run value, Linux XDG `.desktop` entry, honest `unsupported` fallback). `core/autostart_manager.py` builds the relaunch argv (frozen→executable, source→interpreter+main.py, pythonw.exe on Windows) and routes to the adapter. Settings overlay shows a toggle only when the OS supports it and reverts on an unverified change.
- Clipboard intelligence (from Mark-XLIX): copying 10+ chars shows a floating `ClipboardPanel` in `ui.py` with Translate/Summarise/Explain/Fix; the copied text is sent to the assistant only on explicit click, panel auto-dismisses after 8s. Gated by `clipboard_actions_enabled` in `config/settings.json` (default on). Labels + command templates are bilingual.
- Assistant customization (from Mark-XLIX): `config/settings.json` `assistant` block (`assistant_name` default `Jarvis`, `user_name` default empty) read via `core/app_settings.get_assistant_config()`; `main.py::_build_config()` injects an `[IDENTITY]` block into the Gemini system instruction. Editable from the settings overlay; the name fully applies to the Gemini identity after a restart. `memory/config_manager.py` diverged from upstream (it uses secure credential storage), so assistant/user name live in `app_settings`, not `api_keys.json`.
- Messaging (`actions/send_message.py`) drives desktop apps via pyautogui keystrokes (open app → `Cmd/Ctrl+F` search → paste → Enter), gated by a `confirmed` flag (draft unless confirmed). `resolve_messaging_route` allows sending to ANY named app under explicit confirmation — a DeviceProfile-detected app is a normal attempt; an undetected one returns `ok` with `detected=False` (best-effort, unverified — never a fake send). On macOS a confirmed send with `accessibility == "denied"` returns an honest "grant Accessibility" message instead of typing into the void. The real-world blocker is almost always the macOS Accessibility grant (keystrokes are silently dropped without it), not the code — see the permission onboarding below. NOTE: messaging is stored under `apps.messaging` in `device_profile.json` (not top-level `messaging_apps`); `installed_messaging_ids` reads that path.
- Permission onboarding (`core/permissions_manager.py` + adapter contract `permission_status`/`request_permission`/`open_permission_pane`): all five statuses are read from real system APIs (never guessed): Accessibility & Automation via osascript, Screen Recording via Quartz `CGPreflightScreenCaptureAccess`, and Microphone/Camera via `AVCaptureDevice authorizationStatusForMediaType:` — AVFoundation is loaded dynamically through the base `objc` runtime (the pyobjc framework binding is not installed, but the system framework is), mapping authorized→granted, denied/restricted→denied, notDetermined→unknown. Each permission opens the exact `x-apple.systempreferences:` pane / triggers the real prompt (`CGRequestScreenCaptureAccess`); Windows/Linux report `not_required`. `ui.py` `PermissionsOverlay` is a bilingual checklist auto-shown once at startup (background probe, never blocks the Qt thread) when a permission is denied or on first run, and reachable from Settings → PERMISSIONS. `permissions_onboarded` in settings.json prevents re-nagging. No new dependencies (uses the already-present Quartz binding + `open`).
...
- `core/session_context.py` stores runtime-only short-term action context for the current process. It keeps the last 5 meaningful actions, summarizes sensitive parameters, tracks recent browser/app/message/file/media targets, records verified/failed/uncertain/confirmation status, resolves vague follow-up intents, and attaches user corrections. It is now a **universal** action-context layer: a shared `classify_action()` normalizer tags every tool with an `action_category` (app_lifecycle/browser/media/file/messaging/reminder/search/screen/settings/...), `reversible`, `available_followups` and an `undo_action`; a per-category `category_targets` stack keeps each category's last target even after the 5-deep deque evicts it, so an unrelated action never loses another category's target. `resolve_follow_up()` covers universal intents — `app_close`, `media_resume` (davom ettir), `browser_back` (orqaga qayt), `repeat` (yana qil, safe-vs-confirm), `undo` (orqaga qaytar → inverse tool or honest unsupported, never a fake reversal), file edit/move target, `reminder_reschedule`, `open_search_result` — routed and enforced in `main.py::_execute_tool` (not prompt-only), with a stored `_last_executed` for safe repeat.
- `actions/app_control.py` (`close_app`) gracefully quits an application through the platform adapters' new `close_app` method (macOS `quit`, Windows `taskkill` without `/F`, Linux `wmctrl -c`/`pkill -TERM`), verifies it is no longer running, and returns honest verified/uncertain/failed/unsupported statuses. It never force-kills by default; `actions/media_control.py` still never closes apps.
...
- `core/remote_tunnel.py` runs/monitors `cloudflared` for remote-from-anywhere access. `core/power_manager.py` is a cross-platform keep-awake facade over the platform adapters. `core/autostart_manager.py` is the cross-platform launch-at-login facade over the adapters' `autostart_status`/`set_autostart` contract. `core/permissions_manager.py` is the cross-platform OS-permission onboarding facade over the adapters' `permission_status`/`request_permission`/`open_permission_pane` contract. `core/app_settings.py` is the safe read/modify/write layer for non-secret `config/settings.json` keys (`remote_tunnel`, `keep_awake_enabled`, `clipboard_actions_enabled`, `assistant`, `permissions_onboarded`) that preserves unrelated keys. `core/capabilities.py` + `core/macros.py` provide the capability registry and macro store for command automation.
...
Vague follow-up commands such as `o'chir`, `to'xtat`, `yubor`, `yana qil`, `bekor qil`, `shuni yop`, `oldingi ishni davom ettir`, `qayerga yubording?`, and `nima qilding?` must be resolved from recent action context before selecting a tool. Recent browser/app/contact/file/media context has priority over random defaults. The universal layer also resolves `endi yop` (close the recently opened app via `close_app`), `davom ettir` (media resume), `orqaga qayt` (browser back), `orqaga qaytar` (undo the last action via its inverse, or an honest unsupported message — never a fake reversal), `yana qil` (repeat the last safe action; side-effecting actions require confirmation), `tahrir qil`/`boshqa joyga ko'chir` (last file target), and `vaqtini o'zgartir` (reschedule the last reminder, honestly noting the old one is not auto-removed). Per-category targets survive deque eviction, so an unrelated action never loses another category's target.
