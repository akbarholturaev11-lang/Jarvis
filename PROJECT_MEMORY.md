...
- `core/remote_tunnel.py` runs/monitors `cloudflared` for remote-from-anywhere access. `core/power_manager.py` is a cross-platform keep-awake facade over the platform adapters. `core/autostart_manager.py` is the cross-platform launch-at-login facade over the adapters' `autostart_status`/`set_autostart` contract. `core/app_settings.py` is the safe read/modify/write layer for non-secret `config/settings.json` keys (`remote_tunnel`, `keep_awake_enabled`, `clipboard_actions_enabled`, `assistant`) that preserves unrelated keys. `core/capabilities.py` + `core/macros.py` provide the capability registry and macro store for command automation.
